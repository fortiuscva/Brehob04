codeunit 60102 "Format Address 2"
{
    ...
    begin
        ...
        else if not Country.Get(CountryCode)then begin
                Country.Init();
        ...
        IsHandled:=false;
        OnFormatAddrOnAfterGetCountry(AddrArray, Name, Name2, Contact, Addr, Addr2, City, PostCode, County, CountryCode, LanguageCode, IsHandled, Country);
        ...
        if Country."Address Format" = Country."Address Format"::Custom then begin
            ...
            if CustomAddressFormat.FindSet()then repeat case CustomAddressFormat."Field ID" of CompanyInfo.FieldNo(Name): AddrArray[CustomAddressFormat."Line Position" + 1]:=Name;
                    CompanyInfo.FieldNo("Name 2"): AddrArray[CustomAddressFormat."Line Position" + 1]:=Name2;
                    CompanyInfo.FieldNo("Contact Person"): AddrArray[CustomAddressFormat."Line Position" + 1]:=Contact;
                    CompanyInfo.FieldNo(Address): AddrArray[CustomAddressFormat."Line Position" + 1]:=Addr;
                    CompanyInfo.FieldNo("Address 2"): AddrArray[CustomAddressFormat."Line Position" + 1]:=Addr2;
                    CompanyInfo.FieldNo(City): AddrArray[CustomAddressFormat."Line Position" + 1]:=City;
                    CompanyInfo.FieldNo("Post Code"): AddrArray[CustomAddressFormat."Line Position" + 1]:=PostCode;
                    CompanyInfo.FieldNo(County): AddrArray[CustomAddressFormat."Line Position" + 1]:=County;
                    CompanyInfo.FieldNo("Country/Region Code"): AddrArray[CustomAddressFormat."Line Position" + 1]:=Country.Name;
                    else
        ...
        begin
            ...
            IsHandled:=false;
            OnBeforeFormatAddress(Country, AddrArray, Name, Name2, Contact, Addr, Addr2, City, PostCode, County, CountryCode, NameLineNo, Name2LineNo, AddrLineNo, Addr2LineNo, ContLineNo, PostCodeCityLineNo, CountyLineNo, CountryLineNo, IsHandled);
            if IsHandled then exit;
            AddrArray[NameLineNo]:=Name;
            AddrArray[Name2LineNo]:=Name2;
            AddrArray[AddrLineNo]:=Addr;
            AddrArray[Addr2LineNo]:=Addr2;
            case Country."Address Format" of Country."Address Format"::"Post Code+City", Country."Address Format"::"City+County+Post Code", Country."Address Format"::"City+County+New Line+Post Code", Country."Address Format"::"Post Code+City+County", Country."Address Format"::"City+Post Code": UpdateAddrArrayForPostCodeCity(AddrArray, Contact, ContLineNo, Country, CountryLineNo, PostCodeCityLineNo, CountyLineNo, City, PostCode, County);
            Country."Address Format"::"Blank Line+Post Code+City": begin
                if ContLineNo < PostCodeCityLineNo then AddrArray[ContLineNo]:=Contact;
                CompressArray(AddrArray);
                Index:=1;
                InsertText:=1;
                repeat if AddrArray[Index] = '' then begin
                        case InsertText of 2: GeneratePostCodeCity(AddrArray[Index], AddrArray[Index + 1], City, PostCode, County, Country);
                        3: AddrArray[Index]:=Country.Name;
                        4: if ContLineNo > PostCodeCityLineNo then AddrArray[Index]:=Contact;
                        end;
    ...
    begin
        ...
        if Country."Address Format" = Country."Address Format"::Custom then GenerateCustomPostCodeCity(PostCodeCityText, City, PostCode, County, Country)
        else
    ...
    begin
        ...
        if OverMaxStrLen < MaxStrLen(DummyString)then OverMaxStrLen+=1;
        case Country."Address Format" of Country."Address Format"::"Post Code+City": begin
            if PostCode <> '' then PostCodeCityText:=DelStr(PostCode + ' ' + City, OverMaxStrLen)
            else
                PostCodeCityText:=City;
            CountyText:=County;
        end;
        Country."Address Format"::"City+County+Post Code": begin
            CountyText:='';
            if PostCode = '' then begin
                if County = '' then PostCodeCityText:=City
                else
                    PostCodeCityText:=DelStr(City, MaxStrLen(PostCodeCityText) - StrLen(County) - 2) + ', ' + County;
            end
            else if County = '' then PostCodeCityText:=DelStr(City, MaxStrLen(PostCodeCityText) - StrLen(PostCode) - 1) + ', ' + PostCode
                else
                    PostCodeCityText:=DelStr(City, MaxStrLen(PostCodeCityText) - StrLen(PostCode) - StrLen(County) - 4) + ', ' + County + ' ' + PostCode;
        end;
        Country."Address Format"::"City+County+New Line+Post Code": begin
            CountyText:=PostCode;
            if County = '' then PostCodeCityText:=City
            else
                PostCodeCityText:=DelStr(City, MaxStrLen(PostCodeCityText) - StrLen(County) - 2) + ', ' + County;
        end;
        Country."Address Format"::"Post Code+City+County": begin
            if PostCode <> '' then PostCodeCityText:=DelStr(PostCode + ' ' + City + ', ' + County, OverMaxStrLen)
            else
                PostCodeCityText:=DelStr(City + ', ' + County, OverMaxStrLen);
        end;
        Country."Address Format"::"City+Post Code": begin
            if PostCode <> '' then PostCodeCityText:=DelStr(City, MaxStrLen(PostCodeCityText) - StrLen(PostCode) - 1) + ', ' + PostCode
            else
                PostCodeCityText:=City;
            CountyText:=County;
        end;
    ...
    begin
        ...
        if not CustomAddressFormat.FindFirst()then exit;
        ...
        if CustomAddressFormatLine.FindSet()then repeat Counter+=1;
                case CustomAddressFormatLine."Field ID" of CompanyInfo.FieldNo(City): PostCodeCityLine+=City;
                CompanyInfo.FieldNo("Post Code"): PostCodeCityLine+=PostCode;
                CompanyInfo.FieldNo(County): PostCodeCityLine+=County;
                end;
                if Counter < CustomAddressFormatLineQty then if CustomAddressFormatLine.Separator <> '' then PostCodeCityLine+=CustomAddressFormatLine.Separator
                    else
                        PostCodeCityLine+=' ';
    ...
    // changed array below from 8 to 9
    procedure Company(var AddrArray: array[9]of Text[100]; var CompanyInfo: Record "Company Information")
    var
        IsHandled: Boolean;
    begin
        IsHandled:=false;
        ...
        if IsHandled then exit;
        //with CompanyInfo do
        FormatAddr2(AddrArray, CompanyInfo.Name, CompanyInfo."Name 2", '', CompanyInfo.Address, CompanyInfo."Address 2", CompanyInfo.City, CompanyInfo."Post Code", CompanyInfo.County, '', CompanyInfo."Phone No.");
    end;
    // changed above array from 8 to 9
    ...
    begin
        ...
        //with Cust do
        FormatAddr(AddrArray, Cust.Name, Cust."Name 2", Cust.Contact, Cust.Address, Cust."Address 2", Cust.City, Cust."Post Code", Cust.County, Cust."Country/Region Code");
    end;
    ...
    begin
        ...
        if Handled then exit;
        //with Vend do
        FormatAddr(AddrArray, Vend.Name, Vend."Name 2", Vend.Contact, Vend.Address, Vend."Address 2", Vend.City, Vend."Post Code", Vend.County, Vend."Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with BankAcc do
        FormatAddr(AddrArray, BankAcc.Name, BankAcc."Name 2", BankAcc.Contact, BankAcc.Address, BankAcc."Address 2", BankAcc.City, BankAcc."Post Code", BankAcc.County, BankAcc."Country/Region Code");
    end;
    ...
    begin
        ...
        if Handled then exit;
        //with SalesHeader do
        FormatAddr(AddrArray, SalesHeader."Sell-to Customer Name", SalesHeader."Sell-to Customer Name 2", SalesHeader."Sell-to Contact", SalesHeader."Sell-to Address", SalesHeader."Sell-to Address 2", SalesHeader."Sell-to City", SalesHeader."Sell-to Post Code", SalesHeader."Sell-to County", SalesHeader."Sell-to Country/Region Code");
    end;
    ...
    begin
        ...
        if Handled then exit;
        //with SalesHeader do
        FormatAddr(AddrArray, SalesHeader."Bill-to Name", SalesHeader."Bill-to Name 2", SalesHeader."Bill-to Contact", SalesHeader."Bill-to Address", SalesHeader."Bill-to Address 2", SalesHeader."Bill-to City", SalesHeader."Bill-to Post Code", SalesHeader."Bill-to County", SalesHeader."Bill-to Country/Region Code");
    end;
    ...
    begin
        ...
        FormatAddr(AddrArray, SalesHeader."Ship-to Name", SalesHeader."Ship-to Name 2", SalesHeader."Ship-to Contact", SalesHeader."Ship-to Address", SalesHeader."Ship-to Address 2", SalesHeader."Ship-to City", SalesHeader."Ship-to Post Code", SalesHeader."Ship-to County", SalesHeader."Ship-to Country/Region Code");
        if CountryRegion.Get(SalesHeader."Sell-to Country/Region Code")then SellToCountry:=CountryRegion.GetTranslatedName(LanguageCode);
    ...
    begin
        ...
        if IsHandled then exit;
        //with PurchHeader do
        FormatAddr(AddrArray, PurchHeader."Buy-from Vendor Name", PurchHeader."Buy-from Vendor Name 2", PurchHeader."Buy-from Contact", PurchHeader."Buy-from Address", PurchHeader."Buy-from Address 2", PurchHeader."Buy-from City", PurchHeader."Buy-from Post Code", PurchHeader."Buy-from County", PurchHeader."Buy-from Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with PurchHeader do
        FormatAddr(AddrArray, PurchHeader."Pay-to Name", PurchHeader."Pay-to Name 2", PurchHeader."Pay-to Contact", PurchHeader."Pay-to Address", PurchHeader."Pay-to Address 2", PurchHeader."Pay-to City", PurchHeader."Pay-to Post Code", PurchHeader."Pay-to County", PurchHeader."Pay-to Country/Region Code");
    end;
    //changed array below from 8 to 9
    ...
    begin
        ...
        if IsHandled then exit;
        //with PurchHeader do
        FormatAddr2(AddrArray, PurchHeader."Ship-to Name", PurchHeader."Ship-to Name 2", PurchHeader."Ship-to Contact", PurchHeader."Ship-to Address", PurchHeader."Ship-to Address 2", PurchHeader."Ship-to City", PurchHeader."Ship-to Post Code", PurchHeader."Ship-to County", PurchHeader."Ship-to Country/Region Code", PurchHeader."Ship-to Phone No.");
    end;
    procedure PurchHeaderRemitTo(var RemitAddressBuffer: Record "Remit Address Buffer"; var PurchHeader: Record "Purchase Header"): Boolean var
    ...
    begin
        ...
        if PurchHeader."Remit-to Code" <> '' then begin
            ...
            if RemitAddress.IsEmpty()then exit(false);
    ...
    begin
        ...
        FormatAddr(AddrArray, SalesShptHeader."Sell-to Customer Name", SalesShptHeader."Sell-to Customer Name 2", SalesShptHeader."Sell-to Contact", SalesShptHeader."Sell-to Address", SalesShptHeader."Sell-to Address 2", SalesShptHeader."Sell-to City", SalesShptHeader."Sell-to Post Code", SalesShptHeader."Sell-to County", SalesShptHeader."Sell-to Country/Region Code");
    end;
    procedure SalesShptBillTo(var AddrArray: array[8]of Text[100]; ShipToAddr: array[8]of Text[100]; var SalesShptHeader: Record "Sales Shipment Header")Result: Boolean var
        IsHandled: Boolean;
    begin
        IsHandled:=false;
        ...
        FormatAddr(AddrArray, SalesShptHeader."Bill-to Name", SalesShptHeader."Bill-to Name 2", SalesShptHeader."Bill-to Contact", SalesShptHeader."Bill-to Address", SalesShptHeader."Bill-to Address 2", SalesShptHeader."Bill-to City", SalesShptHeader."Bill-to Post Code", SalesShptHeader."Bill-to County", SalesShptHeader."Bill-to Country/Region Code");
        if SalesShptHeader."Bill-to Customer No." <> SalesShptHeader."Sell-to Customer No." then exit(true);
        for i:=1 to ArrayLen(AddrArray)do if ShipToAddr[i] <> AddrArray[i]then exit(true);
        //end;
        exit(false);
    end;
    ...
    begin
        ...
        if Handled then exit;
        //with SalesShptHeader do
        FormatAddr(AddrArray, SalesShptHeader."Ship-to Name", SalesShptHeader."Ship-to Name 2", SalesShptHeader."Ship-to Contact", SalesShptHeader."Ship-to Address", SalesShptHeader."Ship-to Address 2", SalesShptHeader."Ship-to City", SalesShptHeader."Ship-to Post Code", SalesShptHeader."Ship-to County", SalesShptHeader."Ship-to Country/Region Code");
    end;
    ...
    begin
        ...
        FormatAddr(AddrArray, SalesInvHeader."Sell-to Customer Name", SalesInvHeader."Sell-to Customer Name 2", SalesInvHeader."Sell-to Contact", SalesInvHeader."Sell-to Address", SalesInvHeader."Sell-to Address 2", SalesInvHeader."Sell-to City", SalesInvHeader."Sell-to Post Code", SalesInvHeader."Sell-to County", SalesInvHeader."Sell-to Country/Region Code");
    end;
    ...
    begin
        ...
        if Handled then exit;
        //with SalesInvHeader do
        FormatAddr(AddrArray, SalesInvHeader."Bill-to Name", SalesInvHeader."Bill-to Name 2", SalesInvHeader."Bill-to Contact", SalesInvHeader."Bill-to Address", SalesInvHeader."Bill-to Address 2", SalesInvHeader."Bill-to City", SalesInvHeader."Bill-to Post Code", SalesInvHeader."Bill-to County", SalesInvHeader."Bill-to Country/Region Code");
    end;
    procedure SalesInvShipTo(var AddrArray: array[8]of Text[100]; CustAddr: array[8]of Text[100]; var SalesInvHeader: Record "Sales Invoice Header")Result: Boolean var
        IsHandled: Boolean;
    begin
        OnBeforeSalesInvShipTo(AddrArray, SalesInvHeader, IsHandled, Result, CustAddr);
        if IsHandled then exit(Result);
        //with SalesInvHeader do begin
        FormatAddr(AddrArray, SalesInvHeader."Ship-to Name", SalesInvHeader."Ship-to Name 2", SalesInvHeader."Ship-to Contact", SalesInvHeader."Ship-to Address", SalesInvHeader."Ship-to Address 2", SalesInvHeader."Ship-to City", SalesInvHeader."Ship-to Post Code", SalesInvHeader."Ship-to County", SalesInvHeader."Ship-to Country/Region Code");
    ...
    begin
        ...
        FormatAddr(AddrArray, SalesCrMemoHeader."Sell-to Customer Name", SalesCrMemoHeader."Sell-to Customer Name 2", SalesCrMemoHeader."Sell-to Contact", SalesCrMemoHeader."Sell-to Address", SalesCrMemoHeader."Sell-to Address 2", SalesCrMemoHeader."Sell-to City", SalesCrMemoHeader."Sell-to Post Code", SalesCrMemoHeader."Sell-to County", SalesCrMemoHeader."Sell-to Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with SalesCrMemoHeader do
        FormatAddr(AddrArray, SalesCrMemoHeader."Bill-to Name", SalesCrMemoHeader."Bill-to Name 2", SalesCrMemoHeader."Bill-to Contact", SalesCrMemoHeader."Bill-to Address", SalesCrMemoHeader."Bill-to Address 2", SalesCrMemoHeader."Bill-to City", SalesCrMemoHeader."Bill-to Post Code", SalesCrMemoHeader."Bill-to County", SalesCrMemoHeader."Bill-to Country/Region Code");
    end;
    procedure SalesCrMemoShipTo(var AddrArray: array[8]of Text[100]; CustAddr: array[8]of Text[100]; var SalesCrMemoHeader: Record "Sales Cr.Memo Header")Result: Boolean var
        IsHandled: Boolean;
    begin
        IsHandled:=false;
        ...
        FormatAddr(AddrArray, SalesCrMemoHeader."Ship-to Name", SalesCrMemoHeader."Ship-to Name 2", SalesCrMemoHeader."Ship-to Contact", SalesCrMemoHeader."Ship-to Address", SalesCrMemoHeader."Ship-to Address 2", SalesCrMemoHeader."Ship-to City", SalesCrMemoHeader."Ship-to Post Code", SalesCrMemoHeader."Ship-to County", SalesCrMemoHeader."Ship-to Country/Region Code");
        if SalesCrMemoHeader."Sell-to Customer No." <> SalesCrMemoHeader."Bill-to Customer No." then exit(true);
        for i:=1 to ArrayLen(AddrArray)do if AddrArray[i] <> CustAddr[i]then exit(true);
        //end;
        exit(false);
    end;
    ...
    begin
        ...
        FormatAddr(AddrArray, ReturnRcptHeader."Sell-to Customer Name", ReturnRcptHeader."Sell-to Customer Name 2", ReturnRcptHeader."Sell-to Contact", ReturnRcptHeader."Sell-to Address", ReturnRcptHeader."Sell-to Address 2", ReturnRcptHeader."Sell-to City", ReturnRcptHeader."Sell-to Post Code", ReturnRcptHeader."Sell-to County", ReturnRcptHeader."Sell-to Country/Region Code");
    end;
    procedure SalesRcptBillTo(var AddrArray: array[8]of Text[100]; ShipToAddr: array[8]of Text[100]; var ReturnRcptHeader: Record "Return Receipt Header")Result: Boolean var
        IsHandled: Boolean;
    begin
        IsHandled:=false;
        ...
        FormatAddr(AddrArray, ReturnRcptHeader."Bill-to Name", ReturnRcptHeader."Bill-to Name 2", ReturnRcptHeader."Bill-to Contact", ReturnRcptHeader."Bill-to Address", ReturnRcptHeader."Bill-to Address 2", ReturnRcptHeader."Bill-to City", ReturnRcptHeader."Bill-to County", ReturnRcptHeader."Bill-to Post Code", ReturnRcptHeader."Bill-to Country/Region Code");
        if ReturnRcptHeader."Bill-to Customer No." <> ReturnRcptHeader."Sell-to Customer No." then exit(true);
        for i:=1 to ArrayLen(AddrArray)do if AddrArray[i] <> ShipToAddr[i]then exit(true);
        //end;
        exit(false);
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with ReturnRcptHeader do
        FormatAddr(AddrArray, ReturnRcptHeader."Ship-to Name", ReturnRcptHeader."Ship-to Name 2", ReturnRcptHeader."Ship-to Contact", ReturnRcptHeader."Ship-to Address", ReturnRcptHeader."Ship-to Address 2", ReturnRcptHeader."Ship-to City", ReturnRcptHeader."Ship-to Post Code", ReturnRcptHeader."Ship-to County", ReturnRcptHeader."Ship-to Country/Region Code");
    end;
    ...
    begin
        ...
        FormatAddr(AddrArray, PurchRcptHeader."Buy-from Vendor Name", PurchRcptHeader."Buy-from Vendor Name 2", PurchRcptHeader."Buy-from Contact", PurchRcptHeader."Buy-from Address", PurchRcptHeader."Buy-from Address 2", PurchRcptHeader."Buy-from City", PurchRcptHeader."Buy-from County", PurchRcptHeader."Buy-from Post Code", PurchRcptHeader."Buy-from Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with PurchRcptHeader do
        FormatAddr(AddrArray, PurchRcptHeader."Pay-to Name", PurchRcptHeader."Pay-to Name 2", PurchRcptHeader."Pay-to Contact", PurchRcptHeader."Pay-to Address", PurchRcptHeader."Pay-to Address 2", PurchRcptHeader."Pay-to City", PurchRcptHeader."Pay-to Post Code", PurchRcptHeader."Pay-to County", PurchRcptHeader."Pay-to Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with PurchRcptHeader do
        FormatAddr(AddrArray, PurchRcptHeader."Ship-to Name", PurchRcptHeader."Ship-to Name 2", PurchRcptHeader."Ship-to Contact", PurchRcptHeader."Ship-to Address", PurchRcptHeader."Ship-to Address 2", PurchRcptHeader."Ship-to City", PurchRcptHeader."Ship-to Post Code", PurchRcptHeader."Ship-to County", PurchRcptHeader."Ship-to Country/Region Code");
    end;
    ...
    begin
        ...
        FormatAddr(AddrArray, PurchInvHeader."Buy-from Vendor Name", PurchInvHeader."Buy-from Vendor Name 2", PurchInvHeader."Buy-from Contact", PurchInvHeader."Buy-from Address", PurchInvHeader."Buy-from Address 2", PurchInvHeader."Buy-from City", PurchInvHeader."Buy-from Post Code", PurchInvHeader."Buy-from County", PurchInvHeader."Buy-from Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with PurchInvHeader do
        FormatAddr(AddrArray, PurchInvHeader."Pay-to Name", PurchInvHeader."Pay-to Name 2", PurchInvHeader."Pay-to Contact", PurchInvHeader."Pay-to Address", PurchInvHeader."Pay-to Address 2", PurchInvHeader."Pay-to City", PurchInvHeader."Pay-to Post Code", PurchInvHeader."Pay-to County", PurchInvHeader."Pay-to Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with PurchInvHeader do
        FormatAddr(AddrArray, PurchInvHeader."Ship-to Name", PurchInvHeader."Ship-to Name 2", PurchInvHeader."Ship-to Contact", PurchInvHeader."Ship-to Address", PurchInvHeader."Ship-to Address 2", PurchInvHeader."Ship-to City", PurchInvHeader."Ship-to Post Code", PurchInvHeader."Ship-to County", PurchInvHeader."Ship-to Country/Region Code");
    end;
    #if not CLEAN22
    [Obsolete('Replaced by PurchInvRemitTo with dictionaries.', '22.0')]
    procedure PurchInvRemitTo(var AddrArray: array[8]of Text[100]; var PurchInvHeader: Record "Purch. Inv. Header"): Boolean var
        RemitAddress: Record "Remit Address";
    begin
    ...
    #endif 
    procedure PurchInvRemitTo(var RemitAddressBuffer: Record "Remit Address Buffer"; var PurchInvHeader: Record "Purch. Inv. Header"): Boolean var
        RemitAddress: Record "Remit Address";
        IsHandled: Boolean;
    begin
        IsHandled:=false;
    ...
    begin
        ...
        FormatAddr(AddrArray, PurchCrMemoHeader."Buy-from Vendor Name", PurchCrMemoHeader."Buy-from Vendor Name 2", PurchCrMemoHeader."Buy-from Contact", PurchCrMemoHeader."Buy-from Address", PurchCrMemoHeader."Buy-from Address 2", PurchCrMemoHeader."Buy-from City", PurchCrMemoHeader."Buy-from Post Code", PurchCrMemoHeader."Buy-from Address", PurchCrMemoHeader."Buy-from Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with PurchCrMemoHeader do
        FormatAddr(AddrArray, PurchCrMemoHeader."Pay-to Name", PurchCrMemoHeader."Pay-to Name 2", PurchCrMemoHeader."Pay-to Contact", PurchCrMemoHeader."Pay-to Address", PurchCrMemoHeader."Pay-to Address 2", PurchCrMemoHeader."Pay-to City", PurchCrMemoHeader."Pay-to Post Code", PurchCrMemoHeader."Pay-to County", PurchCrMemoHeader."Pay-to Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with PurchCrMemoHeader do
        FormatAddr(AddrArray, PurchCrMemoHeader."Ship-to Name", PurchCrMemoHeader."Ship-to Name 2", PurchCrMemoHeader."Ship-to Contact", PurchCrMemoHeader."Ship-to Address", PurchCrMemoHeader."Ship-to Address 2", PurchCrMemoHeader."Ship-to City", PurchCrMemoHeader."Ship-to Post Code", PurchCrMemoHeader."Ship-to County", PurchCrMemoHeader."Ship-to Country/Region Code");
    end;
    ...
    begin
        ...
        FormatAddr(AddrArray, ReturnShptHeader."Buy-from Vendor Name", ReturnShptHeader."Buy-from Vendor Name 2", ReturnShptHeader."Buy-from Contact", ReturnShptHeader."Buy-from Address", ReturnShptHeader."Buy-from Address 2", ReturnShptHeader."Buy-from City", ReturnShptHeader."Buy-from Post Code", ReturnShptHeader."Buy-from County", ReturnShptHeader."Buy-from Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with ReturnShptHeader do
        FormatAddr(AddrArray, ReturnShptHeader."Pay-to Name", ReturnShptHeader."Pay-to Name 2", ReturnShptHeader."Pay-to Contact", ReturnShptHeader."Pay-to Address", ReturnShptHeader."Pay-to Address 2", ReturnShptHeader."Pay-to City", ReturnShptHeader."Pay-to Post Code", ReturnShptHeader."Pay-to County", ReturnShptHeader."Pay-to Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with ReturnShptHeader do
        FormatAddr(AddrArray, ReturnShptHeader."Ship-to Name", ReturnShptHeader."Ship-to Name 2", ReturnShptHeader."Ship-to Contact", ReturnShptHeader."Ship-to Address", ReturnShptHeader."Ship-to Address 2", ReturnShptHeader."Ship-to City", ReturnShptHeader."Ship-to Post Code", ReturnShptHeader."Ship-to County", ReturnShptHeader."Ship-to Country/Region Code");
    end;
    ...
    begin
        ...
        FormatAddr(AddrArray, CopyStr(Employee.FullName(), 1, 50), '', '', AlternativeAddr.Address, AlternativeAddr."Address 2", AlternativeAddr.City, AlternativeAddr."Post Code", AlternativeAddr.County, AlternativeAddr."Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with Employee do
        FormatAddr(AddrArray, CopyStr(Employee.FullName(), 1, 50), '', '', Employee.Address, Employee."Address 2", Employee.City, Employee."Post Code", Employee.County, Employee."Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with VendBankAcc do
        FormatAddr(AddrArray, VendBankAcc.Name, VendBankAcc."Name 2", VendBankAcc.Contact, VendBankAcc.Address, VendBankAcc."Address 2", VendBankAcc.City, VendBankAcc."Post Code", VendBankAcc.County, VendBankAcc."Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with CustBankAcc do
        FormatAddr(AddrArray, CustBankAcc.Name, CustBankAcc."Name 2", CustBankAcc.Contact, CustBankAcc.Address, CustBankAcc."Address 2", CustBankAcc.City, CustBankAcc."Post Code", CustBankAcc.County, CustBankAcc."Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with RespCenter do
        FormatAddr(AddrArray, RespCenter.Name, RespCenter."Name 2", RespCenter.Contact, RespCenter.Address, RespCenter."Address 2", RespCenter.City, RespCenter."Post Code", RespCenter.County, RespCenter."Country/Region Code");
    end;
    ...
    begin
        ...
        FormatAddr(AddrArray, TransShptHeader."Transfer-from Name", TransShptHeader."Transfer-from Name 2", TransShptHeader."Transfer-from Contact", TransShptHeader."Transfer-from Address", TransShptHeader."Transfer-from Address 2", TransShptHeader."Transfer-from City", TransShptHeader."Transfer-from Post Code", TransShptHeader."Transfer-from County", TransShptHeader."Trsf.-from Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with TransShptHeader do
        FormatAddr(AddrArray, TransShptHeader."Transfer-to Name", TransShptHeader."Transfer-to Name 2", TransShptHeader."Transfer-to Contact", TransShptHeader."Transfer-to Address", TransShptHeader."Transfer-to Address 2", TransShptHeader."Transfer-to City", TransShptHeader."Transfer-to Post Code", TransShptHeader."Transfer-to County", TransShptHeader."Trsf.-to Country/Region Code");
    end;
    ...
    begin
        ...
        FormatAddr(AddrArray, TransRcptHeader."Transfer-from Name", TransRcptHeader."Transfer-from Name 2", TransRcptHeader."Transfer-from Contact", TransRcptHeader."Transfer-from Address", TransRcptHeader."Transfer-from Address 2", TransRcptHeader."Transfer-from City", TransRcptHeader."Transfer-from Post Code", TransRcptHeader."Transfer-from County", TransRcptHeader."Trsf.-from Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with TransRcptHeader do
        FormatAddr(AddrArray, TransRcptHeader."Transfer-to Name", TransRcptHeader."Transfer-to Name 2", TransRcptHeader."Transfer-to Contact", TransRcptHeader."Transfer-to Address", TransRcptHeader."Transfer-to Address 2", TransRcptHeader."Transfer-to City", TransRcptHeader."Transfer-to Post Code", TransRcptHeader."Transfer-to County", TransRcptHeader."Trsf.-to Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with TransHeader do
        FormatAddr(AddrArray, TransHeader."Transfer-from Name", TransHeader."Transfer-from Name 2", TransHeader."Transfer-from Contact", TransHeader."Transfer-from Address", TransHeader."Transfer-from Address 2", TransHeader."Transfer-from City", TransHeader."Transfer-from Post Code", TransHeader."Transfer-from County", TransHeader."Trsf.-from Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with TransHeader do
        FormatAddr(AddrArray, TransHeader."Transfer-to Name", TransHeader."Transfer-to Name 2", TransHeader."Transfer-to Contact", TransHeader."Transfer-to Address", TransHeader."Transfer-to Address 2", TransHeader."Transfer-to City", TransHeader."Transfer-to Post Code", TransHeader."Transfer-to County", TransHeader."Trsf.-to Country/Region Code");
    end;
    procedure ContactAddr(var AddrArray: array[8]of Text[100]; var Cont: Record Contact)
    begin
        ContactAddrAlt(AddrArray, Cont, Cont.ActiveAltAddress(WorkDate()), WorkDate())end;
    ...
    begin
        ...
        case true of AltAddressCode <> '': begin
            //with ContAltAddr do begin
            ContAltAddr.Get(Cont."No.", AltAddressCode);
            FormatAddr(AddrArray, ContAltAddr."Company Name", ContAltAddr."Company Name 2", Cont.Name, ContAltAddr.Address, ContAltAddr."Address 2", ContAltAddr.City, ContAltAddr."Post Code", ContAltAddr.County, ContAltAddr."Country/Region Code");
        end;
        (Cont.Type = Cont.Type::Person) and (Cont."Company No." <> '') and (CompanyAltAddressCode <> '') and RMSetup."Inherit Address Details" and ContIdenticalAddress: begin
            //with ContAltAddr do begin
            ContAltAddr.Get(Cont."Company No.", CompanyAltAddressCode);
            FormatAddr(AddrArray, ContAltAddr."Company Name", ContAltAddr."Company Name 2", Cont.Name, ContAltAddr.Address, ContAltAddr."Address 2", ContAltAddr.City, ContAltAddr."Post Code", ContAltAddr.County, ContAltAddr."Country/Region Code");
        end;
        (Cont.Type = Cont.Type::Person) and (Cont."Company No." <> ''): FormatCompanyContactAddr(AddrArray, Cont, ContCompany)
        else
    ...
    begin
        ...
        if IsHandled then exit;
        FormatAddr(AddrArray, ContCompany.Name, ContCompany."Name 2", Cont.Name, Cont.Address, Cont."Address 2", Cont.City, Cont."Post Code", Cont.County, Cont."Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        FormatAddr(AddrArray, Cont.Name, Cont."Name 2", '', Cont.Address, Cont."Address 2", Cont.City, Cont."Post Code", Cont.County, Cont."Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with ServHeader do
        FormatAddr(AddrArray, ServHeader.Name, ServHeader."Name 2", ServHeader."Contact Name", ServHeader.Address, ServHeader."Address 2", ServHeader.City, ServHeader."Post Code", ServHeader.County, ServHeader."Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with ServHeader do
        FormatAddr(AddrArray, ServHeader."Ship-to Name", ServHeader."Ship-to Name 2", ServHeader."Ship-to Contact", ServHeader."Ship-to Address", ServHeader."Ship-to Address 2", ServHeader."Ship-to City", ServHeader."Ship-to Post Code", ServHeader."Ship-to County", ServHeader."Ship-to Country/Region Code");
    end;
    ...
    begin
        ...
        FormatAddr(AddrArray, ServContract.Name, ServContract."Name 2", ServContract."Contact Name", ServContract.Address, ServContract."Address 2", ServContract.City, ServContract."Post Code", ServContract.County, ServContract."Country/Region Code");
    //end;
    end;
    ...
    begin
        ...
        ServiceContractHeader.CalcFields("Ship-to Name", "Ship-to Name 2", "Ship-to Address", "Ship-to Address 2", "Ship-to Post Code", "Ship-to City", "Ship-to County", "Ship-to Country/Region Code");
        IsHandled:=false;
        OnBeforeServContractShipTo(AddrArray, ServiceContractHeader, IsHandled);
        if IsHandled then exit;
        FormatAddr(AddrArray, ServiceContractHeader."Ship-to Name", ServiceContractHeader."Ship-to Name 2", ServiceContractHeader."Contact Name", ServiceContractHeader."Ship-to Address", ServiceContractHeader."Ship-to Address 2", ServiceContractHeader."Ship-to City", ServiceContractHeader."Ship-to Post Code", ServiceContractHeader."Ship-to County", ServiceContractHeader."Ship-to Country/Region Code");
    //end;
    end;
    ...
    begin
        ...
        FormatAddr(AddrArray, ServiceInvHeader."Bill-to Name", ServiceInvHeader."Bill-to Name 2", ServiceInvHeader."Bill-to Contact", ServiceInvHeader."Bill-to Address", ServiceInvHeader."Bill-to Address 2", ServiceInvHeader."Bill-to City", ServiceInvHeader."Bill-to Post Code", ServiceInvHeader."Bill-to County", ServiceInvHeader."Bill-to Country/Region Code");
    end;
    procedure ServiceInvShipTo(var AddrArray: array[8]of Text[100]; CustAddr: array[8]of Text[100]; var ServiceInvHeader: Record "Service Invoice Header")Result: Boolean var
        IsHandled: Boolean;
    begin
        IsHandled:=false;
        ...
        FormatAddr(AddrArray, ServiceInvHeader."Ship-to Name", ServiceInvHeader."Ship-to Name 2", ServiceInvHeader."Ship-to Contact", ServiceInvHeader."Ship-to Address", ServiceInvHeader."Ship-to Address 2", ServiceInvHeader."Ship-to City", ServiceInvHeader."Ship-to Post Code", ServiceInvHeader."Ship-to County", ServiceInvHeader."Ship-to Country/Region Code");
        if ServiceInvHeader."Customer No." <> ServiceInvHeader."Bill-to Customer No." then exit(true);
        for i:=1 to ArrayLen(AddrArray)do if AddrArray[i] <> CustAddr[i]then exit(true);
        //end;
        exit(false);
    end;
    ...
    begin
        ...
        FormatAddr(AddrArray, ServiceShptHeader."Ship-to Name", ServiceShptHeader."Ship-to Name 2", ServiceShptHeader."Ship-to Contact", ServiceShptHeader."Ship-to Address", ServiceShptHeader."Ship-to Address 2", ServiceShptHeader."Ship-to City", ServiceShptHeader."Ship-to Post Code", ServiceShptHeader."Ship-to County", ServiceShptHeader."Ship-to Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with ServiceShptHeader do
        FormatAddr(AddrArray, ServiceShptHeader.Name, ServiceShptHeader."Name 2", ServiceShptHeader."Contact Name", ServiceShptHeader.Address, ServiceShptHeader."Address 2", ServiceShptHeader.City, ServiceShptHeader."Post Code", ServiceShptHeader.County, ServiceShptHeader."Country/Region Code");
    end;
    procedure ServiceShptBillTo(var AddrArray: array[8]of Text[100]; ShipToAddr: array[8]of Text[100]; var ServiceShptHeader: Record "Service Shipment Header")Result: Boolean var
        IsHandled: Boolean;
    begin
        IsHandled:=false;
        ...
        FormatAddr(AddrArray, ServiceShptHeader."Bill-to Name", ServiceShptHeader."Bill-to Name 2", ServiceShptHeader."Bill-to Contact", ServiceShptHeader."Bill-to Address", ServiceShptHeader."Bill-to Address 2", ServiceShptHeader."Bill-to City", ServiceShptHeader."Bill-to Post Code", ServiceShptHeader."Bill-to County", ServiceShptHeader."Bill-to Country/Region Code");
        if ServiceShptHeader."Bill-to Customer No." <> ServiceShptHeader."Customer No." then exit(true);
        for i:=1 to ArrayLen(AddrArray)do if ShipToAddr[i] <> AddrArray[i]then exit(true);
        //end;
        exit(false);
    end;
    ...
    begin
        ...
        FormatAddr(AddrArray, ServiceCrMemoHeader."Bill-to Name", ServiceCrMemoHeader."Bill-to Name 2", ServiceCrMemoHeader."Bill-to Contact", ServiceCrMemoHeader."Bill-to Address", ServiceCrMemoHeader."Bill-to Address 2", ServiceCrMemoHeader."Bill-to City", ServiceCrMemoHeader."Bill-to Post Code", ServiceCrMemoHeader."Bill-to County", ServiceCrMemoHeader."Bill-to Country/Region Code");
    end;
    procedure ServiceCrMemoShipTo(var AddrArray: array[8]of Text[100]; CustAddr: array[8]of Text[100]; var ServiceCrMemoHeader: Record "Service Cr.Memo Header")Result: Boolean var
        IsHandled: Boolean;
    begin
        IsHandled:=false;
        ...
        FormatAddr(AddrArray, ServiceCrMemoHeader."Ship-to Name", ServiceCrMemoHeader."Ship-to Name 2", ServiceCrMemoHeader."Ship-to Contact", ServiceCrMemoHeader."Ship-to Address", ServiceCrMemoHeader."Ship-to Address 2", ServiceCrMemoHeader."Ship-to City", ServiceCrMemoHeader."Ship-to Post Code", ServiceCrMemoHeader."Ship-to County", ServiceCrMemoHeader."Ship-to Country/Region Code");
        if ServiceCrMemoHeader."Customer No." <> ServiceCrMemoHeader."Bill-to Customer No." then exit(true);
        for i:=1 to ArrayLen(AddrArray)do if AddrArray[i] <> CustAddr[i]then exit(true);
        //end;
        exit(false);
    end;
    ...
    begin
        ...
        FormatAddr(AddrArray, ServiceHeader.Name, ServiceHeader."Name 2", ServiceHeader."Contact Name", ServiceHeader.Address, ServiceHeader."Address 2", ServiceHeader.City, ServiceHeader."Post Code", ServiceHeader.County, ServiceHeader."Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with ServiceHeader do
        FormatAddr(AddrArray, ServiceHeader."Bill-to Name", ServiceHeader."Bill-to Name 2", ServiceHeader."Bill-to Contact", ServiceHeader."Bill-to Address", ServiceHeader."Bill-to Address 2", ServiceHeader."Bill-to City", ServiceHeader."Bill-to Post Code", ServiceHeader."Bill-to County", ServiceHeader."Bill-to Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with ServiceHeader do
        FormatAddr(AddrArray, ServiceHeader."Ship-to Name", ServiceHeader."Ship-to Name 2", ServiceHeader."Ship-to Contact", ServiceHeader."Ship-to Address", ServiceHeader."Ship-to Address 2", ServiceHeader."Ship-to City", ServiceHeader."Ship-to Post Code", ServiceHeader."Ship-to County", ServiceHeader."Ship-to Country/Region Code");
    end;
    procedure PostalBarCode(AddressType: Option): Text[100]begin
        if AddressType = AddressType then exit('');
        exit('');
    end;
    ...
    begin
        ...
        FormatAddr(AddrArray, SalesHeaderArch."Bill-to Name", SalesHeaderArch."Bill-to Name 2", SalesHeaderArch."Bill-to Contact", SalesHeaderArch."Bill-to Address", SalesHeaderArch."Bill-to Address 2", SalesHeaderArch."Bill-to City", SalesHeaderArch."Bill-to Post Code", SalesHeaderArch."Bill-to County", SalesHeaderArch."Bill-to Country/Region Code");
    end;
    procedure SalesHeaderArchShipTo(var AddrArray: array[8]of Text[100]; CustAddr: array[8]of Text[100]; var SalesHeaderArch: Record "Sales Header Archive")Result: Boolean var
        CountryRegion: Record "Country/Region";
        SellToCountry: Code[50];
        IsHandled: Boolean;
    begin
        ...
        FormatAddr(AddrArray, SalesHeaderArch."Ship-to Name", SalesHeaderArch."Ship-to Name 2", SalesHeaderArch."Ship-to Contact", SalesHeaderArch."Ship-to Address", SalesHeaderArch."Ship-to Address 2", SalesHeaderArch."Ship-to City", SalesHeaderArch."Ship-to Post Code", SalesHeaderArch."Ship-to County", SalesHeaderArch."Ship-to Country/Region Code");
        if SalesHeaderArch."Sell-to Customer No." <> SalesHeaderArch."Bill-to Customer No." then exit(true);
        if CountryRegion.Get(SalesHeaderArch."Sell-to Country/Region Code")then SellToCountry:=CountryRegion.GetTranslatedName(LanguageCode);
        for i:=1 to ArrayLen(AddrArray)do if(AddrArray[i] <> CustAddr[i]) and (AddrArray[i] <> '') and (AddrArray[i] <> SellToCountry)then exit(true);
        //end;
        exit(false);
    end;
    ...
    begin
        ...
        FormatAddr(AddrArray, PurchHeaderArch."Buy-from Vendor Name", PurchHeaderArch."Buy-from Vendor Name 2", PurchHeaderArch."Buy-from Contact", PurchHeaderArch."Buy-from Address", PurchHeaderArch."Buy-from Address 2", PurchHeaderArch."Buy-from City", PurchHeaderArch."Buy-from Post Code", PurchHeaderArch."Buy-from County", PurchHeaderArch."Buy-from Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with PurchHeaderArch do
        FormatAddr(AddrArray, PurchHeaderArch."Pay-to Name", PurchHeaderArch."Pay-to Name 2", PurchHeaderArch."Pay-to Contact", PurchHeaderArch."Pay-to Address", PurchHeaderArch."Pay-to Address 2", PurchHeaderArch."Pay-to City", PurchHeaderArch."Pay-to Post Code", PurchHeaderArch."Pay-to County", PurchHeaderArch."Pay-to Country/Region Code");
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        //with PurchHeaderArch do
        FormatAddr(AddrArray, PurchHeaderArch."Ship-to Name", PurchHeaderArch."Ship-to Name 2", PurchHeaderArch."Ship-to Contact", PurchHeaderArch."Ship-to Address", PurchHeaderArch."Ship-to Address 2", PurchHeaderArch."Ship-to City", PurchHeaderArch."Ship-to Post Code", PurchHeaderArch."Ship-to County", PurchHeaderArch."Ship-to Country/Region Code");
    end;
    ...
    begin
        ...
        FormatAddr(AddrArray, ReminderHeader.Name, ReminderHeader."Name 2", ReminderHeader.Contact, ReminderHeader.Address, ReminderHeader."Address 2", ReminderHeader.City, ReminderHeader."Post Code", ReminderHeader.County, ReminderHeader."Country/Region Code");
    end;
    ...
    begin
        ...
        FormatAddr(AddrArray, IssuedReminderHeader.Name, IssuedReminderHeader."Name 2", IssuedReminderHeader.Contact, IssuedReminderHeader.Address, IssuedReminderHeader."Address 2", IssuedReminderHeader.City, IssuedReminderHeader."Post Code", IssuedReminderHeader.County, IssuedReminderHeader."Country/Region Code");
    end;
    ...
    begin
        ...
        if Handled then exit;
        //with Job do
        FormatAddr(AddrArray, Job."Bill-to Name", Job."Bill-to Name 2", Job."Bill-to Contact", Job."Bill-to Address", Job."Bill-to Address 2", Job."Bill-to City", Job."Bill-to Post Code", Job."Bill-to County", Job."Bill-to Country/Region Code");
    end;
    #if not CLEAN22
    ...
    begin
        VendorRemitToAddress(RemitAddress, RemitAddressBuffer);
        FormatAddr(ArrayAddress, RemitAddressBuffer.Name, RemitAddress."Name 2", RemitAddress.Contact, RemitAddress.Address, RemitAddress."Address 2", RemitAddress.City, RemitAddress."Post Code", RemitAddress.County, RemitAddressBuffer."Country/Region Code");
    end;
    ...
    procedure UseCounty(CountryCode: Code[10]): Boolean var
        CountryRegion: Record "Country/Region";
        CustomAddressFormat: Record "Custom Address Format";
        IsHandled: Boolean;
        ReturnValue: Boolean;
    begin
        ...
        if IsHandled then exit(ReturnValue);
        if CountryCode = '' then begin
            GetGLSetup();
            case true of GLSetup."Local Address Format" = GLSetup."Local Address Format"::"City+County+Post Code": exit(true);
            GLSetup."Local Address Format" = GLSetup."Local Address Format"::"City+County+New Line+Post Code": exit(true);
            GLSetup."Local Address Format" = GLSetup."Local Address Format"::"Post Code+City+County": exit(true);
            CustomAddressFormat.UseCounty(''): exit(true);
            end;
        end;
        if CountryRegion.Get(CountryCode)then case true of CountryRegion."Address Format" = CountryRegion."Address Format"::"City+County+Post Code": exit(true);
            CountryRegion."Address Format" = CountryRegion."Address Format"::"City+County+New Line+Post Code": exit(true);
            CountryRegion."Address Format" = CountryRegion."Address Format"::"Post Code+City+County": exit(true);
            CustomAddressFormat.UseCounty(CountryCode): exit(true);
            end;
        exit(false);
    end;
    ...
    begin
        ...
        if IsHandled then exit;
        case Country."Contact Address Format" of Country."Contact Address Format"::First: begin
            NameLineNo:=2;
            Name2LineNo:=3;
            ContLineNo:=1;
            AddrLineNo:=4;
        ...
        Country."Contact Address Format"::"After Company Name": begin
            NameLineNo:=1;
            Name2LineNo:=2;
            ContLineNo:=3;
            AddrLineNo:=4;
        ...
        Country."Contact Address Format"::Last: begin
            NameLineNo:=1;
            Name2LineNo:=2;
            ContLineNo:=8;
            AddrLineNo:=3;
    ...
    procedure FormatAddr2(var AddrArray: array[9]of Text[100]; Name: Text[100]; Name2: Text[100]; Contact: Text[100]; Addr: Text[100]; Addr2: Text[50]; City: Text[50]; PostCode: Code[20]; County: Text[50]; CountryCode: Code[10]; PhoneNo: text[30])
    var
        Country: Record "Country/Region";
        CustomAddressFormat: Record "Custom Address Format";
        InsertText: Integer;
    ...
    begin
        ...
        if CountryCode = '' then begin
            ...
            Country."Contact Address Format":=GLSetup."Local Cont. Addr. Format";
                ...
                Country.TranslateName(LanguageCode);
        IsHandled:=false;
        OnFormatAddrOnAfterGetCountry2(AddrArray, Name, Name2, Contact, Addr, Addr2, City, PostCode, County, CountryCode, LanguageCode, IsHandled, Country, PhoneNo);
        if IsHandled then exit;
        if Country."Address Format" = Country."Address Format"::Custom then begin
            CustomAddressFormat.Reset();
            CustomAddressFormat.SetCurrentKey("Country/Region Code", "Line Position");
            ...
            if CustomAddressFormat.FindSet()then repeat case CustomAddressFormat."Field ID" of CompanyInfo.FieldNo(Name): AddrArray[CustomAddressFormat."Line Position" + 1]:=Name;
                    ...
                    CompanyInfo.FieldNo(County): AddrArray[CustomAddressFormat."Line Position" + 1]:=County;
                    CompanyInfo.FieldNo("Country/Region Code"): AddrArray[CustomAddressFormat."Line Position" + 1]:=Country.Name;
                    CompanyInfo.FieldNo("Phone No."): AddrArray[CustomAddressFormat."Line Position" + 1]:=PhoneNo;
                    else
                        GenerateCustomPostCodeCity(AddrArray[CustomAddressFormat."Line Position" + 1], City, PostCode, County, Country);
                    end;
                until CustomAddressFormat.Next() = 0;
        ...
        begin
            SetLineNos2(Country, NameLineNo, Name2LineNo, AddrLineNo, Addr2LineNo, ContLineNo, PostCodeCityLineNo, CountyLineNo, CountryLineNo, PhoneLineNo);
            IsHandled:=false;
            OnBeforeFormatAddress(Country, AddrArray, Name, Name2, Contact, Addr, Addr2, City, PostCode, County, CountryCode, NameLineNo, Name2LineNo, AddrLineNo, Addr2LineNo, ContLineNo, PostCodeCityLineNo, CountyLineNo, CountryLineNo, IsHandled);
            if IsHandled then exit;
            AddrArray[NameLineNo]:=Name;
    ...
    //Custom procedure modelled after SetLineNos
    local procedure SetLineNos2(Country: Record "Country/Region"; var NameLineNo: Integer; var Name2LineNo: Integer; var AddrLineNo: Integer; var Addr2LineNo: Integer; var ContLineNo: Integer; var PostCodeCityLineNo: Integer; var CountyLineNo: Integer; var CountryLineNo: Integer; var PhoneLineNo: integer)
    var
        IsHandled: Boolean;
    begin
        IsHandled:=false;
    ...
    //add another element - PhoneNo
    [IntegrationEvent(false, false)]
    local procedure OnFormatAddrOnAfterGetCountry2(var AddrArray: array[8]of Text[100]; var Name: Text[100]; var Name2: Text[100]; var Contact: Text[100]; var Addr: Text[100]; var Addr2: Text[50]; var City: Text[50]; var PostCode: Code[20]; var County: Text[50]; var CountryCode: Code[10]; LanguageCode: Code[10]; var IsHandled: Boolean; var Country: Record "Country/Region"; var PhoneNo: text[30])
    begin
    end;
    [IntegrationEvent(false, false)]
