codeunit 60101 "Catalog Item Management 2"
{
    trigger OnRun()
    begin
    end;
    var NewItem: Record Item;
    ...
    begin
        ...
        if not IsHandled then ShowItemCreatedMessage(NonStock2);
        if CheckLicensePermission(DATABASE::"Item Vendor")then NonstockItemVend(NonStock2);
        if CheckLicensePermission(DATABASE::"Item Reference")then NonstockItemReference(NonStock2);
    ...
    begin
        ...
        if NewItem.Get(NonStock2."Item No.")then Error(Text000, NonStock2."Item No.");
    end;
    ...
    begin
        ...
        if not IsHandled then Message(Text001, NonStock2."Item No.");
    end;
    ...
    begin
        ...
        if ItemVend.FindFirst()then exit;
    ...
    begin
        ...
        if NonStock.FindSet()then repeat NonStock."Item No.":='';
    ...
    begin
        ...
        If IsHandled then exit;
        if(SalesLine2."Document Type" in[SalesLine2."Document Type"::"Return Order", SalesLine2."Document Type"::"Credit Memo"])then Error(Text002, SalesLine2."Document Type");
        ...
        if CheckLicensePermission(DATABASE::"Item Vendor")then NonstockItemVend(NonStock);
        if CheckLicensePermission(DATABASE::"Item Reference")then NonstockItemReference(NonStock);
        OnNonStockSalesOnBeforeProgWindowClose(NonStock, NewItem, SalesLine2, IsHandled);
        if GuiAllowed()then ProgWindow.Close();
    end;
    //>> New procedure to pull catalog items into jobs
    procedure NonStockJobs(var JobPlanningLine2: Record "Job Planning Line")
    var
        IsHandled: Boolean;
    begin
        IsHandled:=false;
        OnBeforeNonStockJobs(NonStock, JobPlanningLine2, IsHandled);
        If IsHandled then exit;
        //if (JobPlanningLine2."Document Type" in
        //    [JobPlanningLine2."Document Type"::"Return Order", JobPlanningLine2."Document Type"::"Credit Memo"])
        //then
        //    Error(Text002, JobPlanningLine2."Document Type");
        NonStock.Get(JobPlanningLine2."No.");
        if NonStock."Item No." <> '' then begin
            JobPlanningLine2."No.":=NonStock."Item No.";
            exit;
        end;
        DetermineItemNoAndItemNoSeries(NonStock);
        NonStock.Modify();
        JobPlanningLine2."No.":=NonStock."Item No.";
        OnNonStockJobsOnBeforeInsertItemUnitOfMeasure(NonStock);
        InsertItemUnitOfMeasure(NonStock."Unit of Measure", JobPlanningLine2."No.");
        NewItem.SetRange("No.", JobPlanningLine2."No.");
        if NewItem.FindFirst()then exit;
        if GuiAllowed()then begin
            ProgWindow.Open(Text003 + Text004 + Text005 + Text006 + Text007);
            ProgWindow.Update(1, NonStock."Manufacturer Code");
            ProgWindow.Update(2, NonStock."Vendor No.");
            ProgWindow.Update(3, NonStock."Vendor Item No.");
            ProgWindow.Update(4, JobPlanningLine2."No.");
        end;
        CreateNewItem(NonStock);
        OnNonStockJobsOnAfterCreateNewItem(NewItem);
        if CheckLicensePermission(DATABASE::"Item Vendor")then NonstockItemVend(NonStock);
        if CheckLicensePermission(DATABASE::"Item Reference")then NonstockItemReference(NonStock);
        OnNonStockJobsOnBeforeProgWindowClose(NonStock, NewItem, JobPlanningLine2, IsHandled);
        if GuiAllowed()then ProgWindow.Close();
    end;
    //<<
    ...
    begin
        ...
        if IsHandled then exit;
        if SalesLine2.Nonstock = false then exit;
    ...
    begin
        ...
        if IsHandled then exit;
        if PurchLine2.Nonstock = false then exit;
    ...
    begin
        if ServInvLine2.Nonstock = false then exit;
    ...
    local procedure CheckLicensePermission(TableID: Integer): Boolean var
    ...
    begin
        ...
        if not ItemLedgEntry.IsEmpty()then exit;
        ...
        if not ServInvLine.IsEmpty()then exit;
        BOMComp.SetCurrentKey(Type, "No.");
        BOMComp.SetRange(Type, BOMComp.Type::Item);
        BOMComp.SetRange("No.", Item."No.");
        if not BOMComp.IsEmpty()then exit;
        SalesLineArch.SetCurrentKey(Type, "No.");
        SalesLineArch.SetRange(Type, SalesLineArch.Type::Item);
        SalesLineArch.SetRange("No.", Item."No.");
        if not SalesLineArch.IsEmpty()then exit;
        ProdBOMLine.Reset();
        ProdBOMLine.SetCurrentKey(Type, "No.");
        ProdBOMLine.SetRange(Type, ProdBOMLine.Type::Item);
        ProdBOMLine.SetRange("No.", Item."No.");
        if ProdBOMLine.Find('-')then repeat if ProdBOMHeader.Get(ProdBOMLine."Production BOM No.") and (ProdBOMHeader.Status = ProdBOMHeader.Status::Certified)then exit;
    ...
    begin
        ...
        if IsHandled then exit;
        ItemNo:=NewItem."No.";
        if NewItem."Created From Nonstock Item" then if NewItem.Delete(true)then begin
                NonStock.SetRange("Item No.", ItemNo);
                if NonStock.Find('-')then repeat NonStock."Item No.":='';
                        NonStock."Item No. Series":='';
                        NonStock.Modify();
                    until NonStock.Next() = 0;
            end;
    ...
    begin
        ...
        If not IsHandled then if not ItemUnitOfMeasure.Get(ItemNo, UnitOfMeasureCode)then begin
    ...
    begin
        ...
        case NonstockItemSetupMy."No. Format" of NonstockItemSetupMy."No. Format"::"Vendor Item No.": NonstockItem."Item No.":=NonstockItem."Vendor Item No.";
        NonstockItemSetupMy."No. Format"::"Mfr. + Vendor Item No.": if NonstockItemSetupMy."No. Format Separator" = '' then begin
                if Length1 + Length2 <= 20 then Evaluate(NonstockItem."Item No.", NonstockItem."Manufacturer Code" + NonstockItem."Vendor Item No.")
                else
                    Evaluate(NonstockItem."Item No.", NonstockItem."Manufacturer Code" + NonstockItem."Entry No.");
            end
            else if Length1 + Length2 < 20 then Evaluate(NonstockItem."Item No.", NonstockItem."Manufacturer Code" + NonstockItemSetupMy."No. Format Separator" + NonstockItem."Vendor Item No.")
                else
                    Evaluate(NonstockItem."Item No.", NonstockItem."Manufacturer Code" + NonstockItemSetupMy."No. Format Separator" + NonstockItem."Entry No.");
        NonstockItemSetupMy."No. Format"::"Vendor Item No. + Mfr.": if NonstockItemSetupMy."No. Format Separator" = '' then begin
                if Length1 + Length2 <= 20 then Evaluate(NonstockItem."Item No.", NonstockItem."Vendor Item No." + NonstockItem."Manufacturer Code")
                else
                    Evaluate(NonstockItem."Item No.", NonstockItem."Entry No." + NonstockItem."Manufacturer Code");
            end
            else if Length1 + Length2 < 20 then Evaluate(NonstockItem."Item No.", NonstockItem."Vendor Item No." + NonstockItemSetupMy."No. Format Separator" + NonstockItem."Manufacturer Code")
                else
                    Evaluate(NonstockItem."Item No.", NonstockItem."Entry No." + NonstockItemSetupMy."No. Format Separator" + NonstockItem."Manufacturer Code");
        NonstockItemSetupMy."No. Format"::"Entry No.": NonstockItem."Item No.":=NonstockItem."Entry No.";
        NonstockItemSetupMy."No. Format"::"Item No. Series": GetItemNoFromNoSeries(NonstockItem);
        end;
    ...
    begin
        ...
        //NoSeriesMgt.GetNextNo(NonstockItem."Item No. Series", 0D, NonstockItem."Item No.", NonstockItem."Item No. Series");
    ...
    begin
        ...
        if Item."Costing Method" = Item."Costing Method"::Standard then Item."Standard Cost":=NonstockItem."Negotiated Cost";
    ...
    //>>New Integegration event
    [IntegrationEvent(false, false)]
    local procedure OnBeforeNonStockJobs(var NonStockItem: Record "Nonstock Item"; var JobPlanningLine: Record "Job Planning Line"; var IsHandled: Boolean)
    begin
    end;
    //<<
    [IntegrationEvent(false, false)]
    ...
    [IntegrationEvent(false, false)]
    local procedure OnNonStockJobsOnAfterCreateNewItem(var NewItem: Record Item)
    begin
    end;
    //<<
    [IntegrationEvent(false, false)]
    ...
    //>> New integtration event
    [IntegrationEvent(false, false)]
    local procedure OnNonStockJobsOnBeforeInsertItemUnitOfMeasure(var NonStockItem: Record "Nonstock Item")
    begin
    end;
    //<<
    [IntegrationEvent(false, false)]
    ...
    //>> New integration event
    [IntegrationEvent(false, false)]
    local procedure OnNonStockJobsOnBeforeProgWindowClose(var NonStockItem: Record "Nonstock Item"; var NewItem: Record Item; JobPlanningLine2: Record "Job Planning Line"; var IsHandled: Boolean)
    begin
    end;
    //<<
    [IntegrationEvent(false, false)]
